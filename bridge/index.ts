...
const VisaraNative = requireNativeModule('Visara');
...
export type EntityType =
  ...
  | 'socialHandle'
  | 'address'
  | `custom:${string}`;   // mirrors Swift's .custom(String)
...
export type ProviderType =
  | 'foundation-models'   // Apple on-device AI (iOS 26+, free)
  | 'claude'              // Anthropic Claude API
  | 'gemini'              // Google Gemini API
  | 'built-in';           // NSDataDetector (zero-config, always free)
...
export interface VisaraResult {
  ...
  // Convenience arrays — filtered by type
  urls: string[];
  phones: string[];
  emails: string[];
  dates: string[];
 ...
 * });
 */
export function configure(options: VisaraOptions): void {
 ...
 * }
 */
export async function scan(imagePath: string): Promise<VisaraResult> {
